...
void clear_memory(char **patterns, char **files, int count_pattern,
                  int count_files);
void flag_o_with_v(int count_pattern, FILE *f, char **patterns, int cflags1,
                   char *str, int *correct, int count_files, char **files,
                   flags *options, int i);
void flag_o_without_v(int count_pattern, FILE *f, char **patterns, int cflags1,
                      char *str, int *correct, int count_files, char **files,
                      flags *options, int i);
void flag_c_and_l(int count_pattern, FILE *f, char **patterns, int cflags1,
                  char *str, int *correct, int count_files, char **files,
                  flags *options, int i);
void flag_c_with_v(int count_pattern, FILE *f, char **patterns, int cflags1,
                   char *str, int *correct, int count_files, char **files,
                   flags *options, int i);
void flag_c_without_v(int count_pattern, FILE *f, char **patterns, int cflags1,
                      char *str, int *correct, int count_files, char **files,
                      flags *options, int i);
void flag_l(int count_pattern, FILE *f, char **patterns, int cflags1, char *str,
            int *correct, char **files, flags *options, int i);
void flag_without_v(int count_pattern, FILE *f, char **patterns, int cflags1,
                    char *str, int *correct, int count_files, char **files,
                    flags *options, int i);
void flag_with_v(int count_pattern, FILE *f, char **patterns, int cflags1,
                 char *str, int *correct, int count_files, char **files,
                 flags *options, int i);
...
void search(flags *options, char **patterns, int count_pattern, int count_files,
  ...
  if (options->o && !(options->c) && !(options->l) && (options->v)) {
    flag_o_with_v(count_pattern, f, patterns, cflags1, str, &correct,
                  count_files, files, options, i);
  } else if (options->o && !(options->c) && !(options->l) && !(options->v)) {
    flag_o_without_v(count_pattern, f, patterns, cflags1, str, &correct,
                     count_files, files, options, i);
  } else if (options->c && options->l) {
    flag_c_and_l(count_pattern, f, patterns, cflags1, str, &correct,
                 count_files, files, options, i);
  } else if (options->c && !(options->l) && !(options->v)) {
    flag_c_without_v(count_pattern, f, patterns, cflags1, str, &correct,
                     count_files, files, options, i);
  } else if (options->c && !(options->l) && (options->v)) {
    flag_c_with_v(count_pattern, f, patterns, cflags1, str, &correct,
                  count_files, files, options, i);
  } else if (!(options->c) && options->l) {
    flag_l(count_pattern, f, patterns, cflags1, str, &correct, files, options,
           i);
  } else if (!(options->v)) {
    flag_without_v(count_pattern, f, patterns, cflags1, str, &correct,
                   count_files, files, options, i);
  } else if (options->v) {
    flag_with_v(count_pattern, f, patterns, cflags1, str, &correct, count_files,
                files, options, i);
  }
}
